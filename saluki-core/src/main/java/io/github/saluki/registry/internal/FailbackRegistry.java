...
public abstract class FailbackRegistry extends AbstractRegistry {
  ...
  private final ScheduledExecutorService retryExecutor = Executors.newScheduledThreadPool(1,
      new NamedThreadFactory("SalukiRegistryFailedRetryTimer", true));
  ...
  private void retry() {
    ...
    if (!failedSubscribed.isEmpty()) {
      Map<GrpcURL, Set<NotifyListener.NotifyServiceListener>> failed =
          new HashMap<GrpcURL, Set<NotifyListener.NotifyServiceListener>>(failedSubscribed);
      for (Map.Entry<GrpcURL, Set<NotifyListener.NotifyServiceListener>> entry : new HashMap<GrpcURL, Set<NotifyListener.NotifyServiceListener>>(
          failed).entrySet()) {
      ...
      if (failed.size() > 0) {
        ...
        try {
          for (Map.Entry<GrpcURL, Set<NotifyListener.NotifyServiceListener>> entry : failed
              .entrySet()) {
    ...
    if (!failedUnsubscribed.isEmpty()) {
      Map<GrpcURL, Set<NotifyListener.NotifyServiceListener>> failed =
          new HashMap<GrpcURL, Set<NotifyListener.NotifyServiceListener>>(failedUnsubscribed);
      for (Map.Entry<GrpcURL, Set<NotifyListener.NotifyServiceListener>> entry : new HashMap<GrpcURL, Set<NotifyListener.NotifyServiceListener>>(
          failed).entrySet()) {
        if (entry.getValue() == null || entry.getValue().size() == 0) {
          failed.remove(entry.getKey());
    ...
    if (!failedNotified.isEmpty()) {
      Map<GrpcURL, Map<NotifyListener.NotifyServiceListener, List<GrpcURL>>> failed =
          new HashMap<GrpcURL, Map<NotifyListener.NotifyServiceListener, List<GrpcURL>>>(
              failedNotified);
      for (Map.Entry<GrpcURL, Map<NotifyListener.NotifyServiceListener, List<GrpcURL>>> entry : new HashMap<GrpcURL, Map<NotifyListener.NotifyServiceListener, List<GrpcURL>>>(
          failed).entrySet()) {
        if (entry.getValue() == null || entry.getValue().size() == 0) {
          failed.remove(entry.getKey());
        }
      ...
      if (failed.size() > 0) {
        ...
        try {
          for (Map.Entry<GrpcURL, Map<NotifyListener.NotifyServiceListener, List<GrpcURL>>> entrySub : failed
              .entrySet()) {
            GrpcURL subscribeUrl = entrySub.getKey();
            Map<NotifyListener.NotifyServiceListener, List<GrpcURL>> values = entrySub.getValue();
            for (Map.Entry<NotifyListener.NotifyServiceListener, List<GrpcURL>> entry : values
                .entrySet()) {
              try {
                NotifyListener.NotifyServiceListener listener = entry.getKey();
                List<GrpcURL> urls = entry.getValue();
                listener.notify(subscribeUrl, urls);
          ...
          logger.warn(
              "Failed to retry notify " + failed + ", waiting for again, cause: " + t.getMessage(),
              t);
        }
      }
    }
  ...
  public void register(GrpcURL url) {
    ...
    try {
      ...
      logger.error("Failed to uregister " + url + ", waiting for retry, cause: " + e.getMessage(),
          e);
  ...
  public void subscribe(GrpcURL url, NotifyListener.NotifyServiceListener listener) {
    ...
    try {
      ...
      logger.error("Failed to subscribe " + url + ", waiting for retry, cause: " + e.getMessage(),
          e);
  ...
  @Override
  protected void notify(GrpcURL url, NotifyListener.NotifyServiceListener listener,
      List<GrpcURL> urls) {
    if (url == null) {
  ...
  protected void doNotify(GrpcURL url, NotifyListener.NotifyServiceListener listener,
      List<GrpcURL> urls) {
  ...
  protected void recover() throws Exception {
    ...
    // subscribe
    Map<GrpcURL, Set<NotifyListener.NotifyServiceListener>> recoverSubscribed =
        Maps.newHashMap(getSubscribed());
    if (!recoverSubscribed.isEmpty()) {
      if (logger.isInfoEnabled()) {
        logger.info("Recover subscribe url " + recoverSubscribed.keySet());
      }
      for (Map.Entry<GrpcURL, Set<NotifyListener.NotifyServiceListener>> entry : recoverSubscribed
          .entrySet()) {
        GrpcURL url = entry.getKey();
