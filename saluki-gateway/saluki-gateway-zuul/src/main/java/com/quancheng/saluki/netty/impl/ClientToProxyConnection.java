 ...
 * ProxyToServerConnection goes through its own lifecycle of connects and disconnects, with
 ...
 * 
 * <p>
 * As the ProxyToServerConnections receive responses from their servers, they feed these back to the
 * client by calling
...
public class ClientToProxyConnection extends ProxyConnection<HttpRequest> {
   ...
   * 
   * <p>
   * If we don't yet have a {@link ProxyToServerConnection} for the desired server, this takes care
   * of creating it.
   * </p>
   * 
   * <p>
   ...
   * 
   * @return true if we're falling back to a another chained proxy (or direct connection) and trying
   *         again
   */
   ...
   * server here.
   * 
   * A {@link ChannelPipeline} invokes the read (Inbound) handlers in ascending ordering of the list
   * and then the write (Outbound) handlers in descending ordering.
   * 
   * Regarding the Javadoc of {@link HttpObjectAggregator} it's needed to have the
   * {@link HttpResponseEncoder} or {@link io.netty.handler.codec.http.HttpRequestEncoder} before
   ...
   * closed.
   * 
   * This in part determines if we should close the connection. Here's the relevant section of RFC
   * 2616:
   * 
   ...
   * Connection: close
   * 
   * in either the request or the response header fields indicates that the connection SHOULD NOT be
   * considered `persistent' (section 8.1) after the current request/response is complete."
   * 
  ...
  private void modifyResponseHeadersToReflectProxying(HttpResponse httpResponse) {
    if (!proxyServer.isTransparent()) {
       ...
       * 
       * A received message that does not have a Date header field MUST be assigned one by the
       * recipient if the message will be cached by that recipient or gatewayed via a protocol which
       * requires a Date.
       */
   ...
   * through the {@link HttpFilter#proxyToClientResponse(HttpObject)} filter method before writing
   ...
   * 
   * We track statistics on bytes, requests and responses by adding handlers at the appropriate
