...
public class Http1ConnectionCloseHandler extends ChannelDuplexHandler {
  ...
  protected void closeChannel(ChannelHandlerContext ctx, ConnectionCloseType evt,
    ...
    if (channel.isActive()) {
      ...
      ctx.executor().schedule(() -> {
        ...
        if (channel.isActive()) {
          ...
          // If there is still an inflight request, then don't close the conn now. Instead assume
          ...
          if (requestInflight.get()) {
            ...
            LOG.debug("gracefully: firing graceful_shutdown event to close connection. channel="
                + channelId);
            ctx.close(promise);
          }
