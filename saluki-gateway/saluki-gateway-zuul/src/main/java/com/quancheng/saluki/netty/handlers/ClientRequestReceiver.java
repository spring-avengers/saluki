...
public class ClientRequestReceiver extends ChannelDuplexHandler {
  ...
  public void channelRead(final ChannelHandlerContext ctx, Object msg) throws Exception {
    if (msg instanceof HttpRequest) {
      ...
      if (clientRequest.decoderResult().isFailure()) {
        String errorMsg = "Invalid http request. " + "clientRequest = " + clientRequest.toString()
            + ", uri = " + String.valueOf(clientRequest.uri()) + ", info = "
    ...
    if (evt instanceof CompleteEvent) {
      ...
      if (reason != SESSION_COMPLETE && zuulRequest != null) {
        ...
        if (clientRequest != null) {
          LOG.warn("Client {} request UUID {} to {} completed with reason = {}, {}",
              clientRequest.method(), zuulCtx.getUUID(), clientRequest.uri(), reason.name(),
              ChannelUtils.channelInfoForLogging(ctx.channel()));
        }
  ...
  private HttpRequestMessage buildZuulHttpRequest(final HttpRequest nativeRequest,
    ...
    final HttpRequestMessage request = new HttpRequestMessageImpl(context, protocol,
        nativeRequest.method().asciiName().toString().toLowerCase(), path,
        copyQueryParams(nativeRequest), copyHeaders(nativeRequest), clientIp, scheme, port,
        serverName);
