...
public abstract class AbstrHttpConnectionExpiryHandler extends ChannelOutboundHandlerAdapter {
  ...
  protected final static CachedDynamicLongProperty MAX_EXPIRY_DELTA =
  ...
  public AbstrHttpConnectionExpiryHandler(ConnectionCloseType connectionCloseType,
      int maxRequestsUnderBrownout, int maxRequests, int maxExpiry) {
  ...
  protected boolean isConnectionExpired(ChannelHandlerContext ctx) {
    ...
    if (expired) {
      ...
      LOG.info("Connection is expired. requestCount={}, lifetime={}, {}", requestCount, lifetime,
