...
public abstract class AbstractRegistry implements Registry {
  ...
  private final ConcurrentMap<GrpcURL, Set<NotifyListener.NotifyServiceListener>> subscribed =
  ...
  public List<GrpcURL> discover(GrpcURL url) {
    ...
    if (notifiedUrls != null && notifiedUrls.size() > 0) {
      ...
      NotifyListener.NotifyServiceListener listener = new NotifyListener.NotifyServiceListener() {
        ...
        public void notify(GrpcURL url, List<GrpcURL> urls) {
          reference.set(urls);
        }
      };
  ...
  protected void notify(List<GrpcURL> providerUrls) {
    if (providerUrls == null || providerUrls.isEmpty())
      return;
    for (Map.Entry<GrpcURL, Set<NotifyListener.NotifyServiceListener>> entry : getSubscribed()
        .entrySet()) {
  ...
  protected void notify(GrpcURL subscribedUrl, NotifyListener.NotifyServiceListener listener,
    ...
    notifyExecutor.submit(new Runnable() {
      ...
      @Override
      public void run() {
        listener.notify(subscribedUrl, providerUrls);
      }
    });
  }
