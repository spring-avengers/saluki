...
import io.github.saluki.registry.NotifyListener.NotifyRouterListener;
...
public class ConsulRegistry extends FailbackRegistry {
  ...
  public List<GrpcURL> discover(GrpcURL url) {
    ...
    try {
      Map<String, List<GrpcURL>> providerUrls =
          serviceCache.get(group, new Callable<Map<String, List<GrpcURL>>>() {
  ...
  @Override
  public void unsubscribe(GrpcURL url, NotifyRouterListener listener) {
    String serviceKey = url.getServiceKey();
    notifyRouterListeners.get(serviceKey).remove(listener);
  }
  ...
  private class RouterLookUper extends Thread {
    ...
    public RouterLookUper(GrpcURL url) {
      this.url = url;
      this.serviceKey = url.getServiceKey();
    }

    private String lookupRouterUpdate(GrpcURL url) {
      Long lastConsulIndexId =
          lookupGroupRouters.get(serviceKey) == null ? 0L : lookupGroupRouters.get(serviceKey);
      String routePath = GrpcURLUtils.toRoutePath(url);
      ConsulRouterResp consulResp = client.lookupRouterMessage(routePath, lastConsulIndexId);
      if (consulResp != null) {
        String routerMessages = consulResp.getSalukiConsulRouter();
        lookupGroupRouters.put(serviceKey, consulResp.getConsulIndex());
    ...
    public void run() {
      while (true) {
        try {
          String groupRouterMessages = lookupRouterUpdate(url);
          Set<NotifyListener.NotifyRouterListener> listeners =
              notifyRouterListeners.get(serviceKey);
          if (listeners != null) {
            for (NotifyListener.NotifyRouterListener listener : listeners) {
              notifyExecutor.submit(new Runnable() {
                ...
                @Override
                public void run() {
                  listener.notify(url, groupRouterMessages);
                }
              });
            }
          }
