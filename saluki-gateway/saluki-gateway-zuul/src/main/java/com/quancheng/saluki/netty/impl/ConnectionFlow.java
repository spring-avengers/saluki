...
class ConnectionFlow {
   ...
   *        {@link ProxyToServerConnection} and that is used for synchronizing the reader and writer
   *        threads that are both involved during the establishing of a connection.
   */
   ...
   * {@link ProxyToServerConnection} are passed to this method, which passes it on to
   * {@link ConnectionFlowStep#read(ConnectionFlow, Object)} for the current
   ...
   * <li>On completion of the {@link Future} returned by {@link ConnectionFlowStep#execute()}, check
   * the success.</li>
  ...
  /**
   * Called when the flow fails at some {@link ConnectionFlowStep}. Disconnects the
   * {@link ProxyToServerConnection} and informs the {@link ClientToProxyConnection} that our
   * connection failed.
   */
  @SuppressWarnings("unchecked")
      ...
      public void operationComplete(Future future) throws Exception {
        synchronized (connectLock) {
          if (!clientConnection.serverConnectionFailed(serverConnection, lastStateBeforeFailure,
            ...
            // We are not retrying our connection, let anyone waiting for a connection know that
            // we're done
            notifyThreadsWaitingForConnection();
          }
        }
