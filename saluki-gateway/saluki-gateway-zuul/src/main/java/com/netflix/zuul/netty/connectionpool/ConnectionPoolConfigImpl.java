...
import com.quancheng.saluki.client.config.IClientConfigKey;
import com.quancheng.saluki.config.CachedDynamicBooleanProperty;
...
public class ConnectionPoolConfigImpl implements ConnectionPoolConfig {
  ...
  public ConnectionPoolConfigImpl(final String originName, IClientConfig clientConfig) {
    ...
    this.MAX_REQUESTS_PER_CONNECTION =
        new CachedDynamicIntProperty(originName + ".netty.client.maxRequestsPerConnection", 1000);

    // NOTE that the each eventloop has it's own connection pool per host, and this is applied per
    // event-loop.
    this.PER_SERVER_WATERLINE =
        new CachedDynamicIntProperty(originName + ".netty.client.perServerWaterline", 4);

    this.SOCKET_KEEP_ALIVE =
        new CachedDynamicBooleanProperty(originName + ".netty.client.TcpKeepAlive", false);
    this.TCP_NO_DELAY =
        new CachedDynamicBooleanProperty(originName + ".netty.client.TcpNoDelay", false);
    this.WRITE_BUFFER_HIGH_WATER_MARK = new CachedDynamicIntProperty(
        originName + ".netty.client.WriteBufferHighWaterMark", 32 * 1024);
    this.WRITE_BUFFER_LOW_WATER_MARK = new CachedDynamicIntProperty(
        originName + ".netty.client.WriteBufferLowWaterMark", 8 * 1024);
  ...
  public int getConnectTimeout() {
    return clientConfig.getPropertyAsInteger(IClientConfigKey.Keys.ConnectTimeout,
  ...
  public int maxConnectionsPerHost() {
    return clientConfig.getPropertyAsInteger(IClientConfigKey.Keys.MaxConnectionsPerHost,
  ...
  public int getIdleTimeout() {
    return clientConfig.getPropertyAsInteger(IClientConfigKey.Keys.ConnIdleEvictTimeMilliSeconds,
  ...
  public int getTcpReceiveBufferSize() {
    return clientConfig.getPropertyAsInteger(IClientConfigKey.Keys.ReceiveBufferSize,
        DEFAULT_BUFFER_SIZE);
  }

  @Override
  public int getTcpSendBufferSize() {
    return clientConfig.getPropertyAsInteger(IClientConfigKey.Keys.SendBufferSize,
        DEFAULT_BUFFER_SIZE);
  }
