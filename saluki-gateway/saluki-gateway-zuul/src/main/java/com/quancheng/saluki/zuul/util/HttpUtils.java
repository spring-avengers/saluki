...
import com.quancheng.saluki.zuul.message.http.HttpRequestInfo;
...
public class HttpUtils {
   ...
   *
   * Uses the "x-forwarded-for" HTTP header if available, otherwise uses the remote IP of requester.
   *
  ...
  public static Integer getContentLengthIfPresent(ZuulMessage msg) {
    final String contentLengthValue = msg.getHeaders().getFirst(HttpHeaderNames.CONTENT_LENGTH);
    if (StringUtils.isNotEmpty(contentLengthValue) && StringUtils.isNumeric(contentLengthValue)) {
      try {
        ...
        LOG.info("Invalid Content-Length header value on request. " + "value = "
            + String.valueOf(contentLengthValue));
      }
    }
    return null;
  }
