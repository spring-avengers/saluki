 ...
 * considered connected and moves into {@link ConnectionState#AWAITING_INITIAL}. The Channel is
 * recorded at this time for later referencing.</li>
 * <li>{@link #disconnected()} - When the underlying channel goes inactive, the ProxyConnection
 * moves into {@link ConnectionState#DISCONNECTED}</li>
 ...
 * </p>
 * 
 * @param <I> the type of "initial" message. This will be either {@link HttpResponse} or
 *        {@link HttpRequest}.
 */
abstract class ProxyConnection<I extends HttpObject> extends SimpleChannelInboundHandler<Object> {
    ...
    switch (getCurrentState()) {
      case AWAITING_INITIAL:
        if (httpObject instanceof HttpMessage) {
          ...
          // AWAITING_INITIAL
          // state if the proxy responded to an earlier request with a 502 or 504 response, or a
          // short-circuit
          // response from a filter. The client may have sent some chunked HttpContent associated
          // with the request
  ...
  /**
   * Implement this to handle reading the initial object (e.g. {@link HttpRequest} or
   * {@link HttpResponse}).
   * 
   * @param httpObject
   * @return
   ...
   * 
   * @return Future<Void> for when we're done disconnecting. If we weren't connected, this returns
   *         null.
   */
   ...
   * 
   * By default, no-op filters are returned by DefaultHttpProxyServer. Subclasses of ProxyConnection
