 ...
 *
 * There are methods for getting and setting headers by String AND by HeaderName. When possible, use
 * the HeaderName variants and cache the HeaderName instances somewhere, to avoid case-insensitive
 * String comparisons.
 *
 * User: michaels@netflix.com Date: 2/20/15 Time: 3:13 PM
 */
public class Headers implements Cloneable {
  ...
  private boolean delegatePut(HeaderName hn, String value) {
    return delegate.put(hn, HttpUtils.stripMaliciousHeaderChars(value));
  }
  ...
  public Collection<Header> entries() {
    return delegate.entries().stream().map(entry -> new Header(entry.getKey(), entry.getValue()))
